...
ardSer = serial.Serial("/dev/tty.usbmodem1411", timeout=1)  # 9600,8,N,1
...
while True:
    ...
    serDat = (ardSer.read(ardSer.inWaiting()-1)) #Maybe read() is deprecated...
    #print serDat
    #serDat = str(serDat) #Instead maybe try serDat.decode('utf-8')
    #print serDat
    ...
    for i in range(6):
        ...
        if i==4 or i==5:
            ...
            #port = mots[i]
            if i==4:
                wrist_servo.move_angle(ang)
            if i==5:
                claw_servo.move_angle(ang)

        print("{}: {} => {:.4f}".format(i, armDat[i], ang))  # debug
